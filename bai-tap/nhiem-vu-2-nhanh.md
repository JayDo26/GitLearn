...
2. Từ nhánh của em, tạo một nhánh con để thử nghiệm:
   ...
   ```bash
   git checkout -b hoc-sinh/ten-cua-em-thu-nghiem
   ```
...
5. Chuyển qua lại vài lần để thấy rõ nội dung file thay đổi theo nhánh:
   ...
   ```bash
   git checkout hoc-sinh/ten-cua-em-thu-nghiem   # co dong So thich
   git checkout hoc-sinh/ten-cua-em              # khong co
   ```
...
- `git checkout -b <ten>` = tạo mới + chuyển sang.
- `git checkout <ten>` = chuyển sang nhánh đã có.
